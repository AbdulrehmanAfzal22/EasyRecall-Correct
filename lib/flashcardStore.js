...
import { getFlashcardsFromFirestore } from "./firebaseStore";

export async function loadFlashcards() {
  if (typeof window === "undefined") return null;
  try {
    const local = JSON.parse(localStorage.getItem(KEYS.CARDS));
    if (local && local.cards && local.cards.length) return local;
    // Fallback to Firestore if logged in
    if (auth.currentUser?.uid) {
      const remote = await getFlashcardsFromFirestore(auth.currentUser.uid);
      if (remote && remote.cards && remote.cards.length) {
        localStorage.setItem(KEYS.CARDS, JSON.stringify(remote));
        return remote;
      }
    }
    return null;
