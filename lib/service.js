...
export async function saveDocument(uid, {
  ...
  // 0. Delete previous document if exists (enforce only one document per user)
  // Load all documents for user
  const q = query(userCol(uid), orderBy("createdAt", "desc"));
  const snap = await getDocs(q);
  if (!snap.empty) {
    // Delete the most recent document (or all, to be safe)
    for (const d of snap.docs) {
      const data = d.data();
      const docId = d.id;
      const prevStoragePath = data.storagePath;
      await deleteDocument(uid, docId, prevStoragePath);
    }
  }
